...
export interface SmoothLightThemeBackgroundProps {
  className?: string;
...
interface LightSmokeBackgroundProps {
  speed?: number;
  intensity?: number;
...
export const LIGHT_BACKGROUND_ROOT_CLASS = 'light relative min-h-screen bg-[#f8f9fa] text-zinc-900 selection:bg-black selection:text-white font-sans antialiased overflow-x-clip';
export const LIGHT_BACKGROUND_BASE_CLASS = 'fixed inset-0 bg-[#f8f9fa] -z-20 pointer-events-none transition-colors duration-300';
export const LIGHT_STACK_SECTION_BACKGROUND_CLASS = 'relative w-full min-h-[100svh] bg-[#f8f9fa] border-t-0 shadow-none';
export const LIGHT_STICKY_SECTION_BACKGROUND_CLASS = 'sticky top-0 h-[100svh] w-full overflow-hidden bg-[#f8f9fa] border-t-0 shadow-none';
export const LIGHT_PARTICLE_CANVAS_CLASS = 'fixed inset-0 pointer-events-none z-[-1]';
export const LIGHT_RADIAL_SOFT_CENTER_CLASS = 'absolute top-0 left-1/2 -translate-x-1/2 w-full max-w-7xl h-screen bg-[radial-gradient(ellipse_at_top,_rgba(0,0,0,0.03)_0%,_transparent_60%)] pointer-events-none -z-10';

export const LIGHT_HERO_READABILITY_OVERLAY_STYLE: React.CSSProperties = {
  background: 'radial-gradient(ellipse 90% 70% at 50% 45%, rgba(255,255,255,0.0) 0%, rgba(244,245,247,0.35) 100%)',
};
...
void main() {
  ...
  vec3 color = vec3(1.0) - u_color * smoke;
...
const LIGHT_BACKGROUND_CSS = `
.smooth-light-theme-background {
  position: fixed;
  inset: 0;
  z-index: -20;
  width: 100%;
...
.smooth-light-theme-background__base {
  position: absolute;
  inset: 0;
  z-index: 0;
  background-color: #f8f9fa;
...
.smooth-light-theme-background__aurora {
  position: absolute;
  inset: -20% -10%;
  z-index: 1;
  pointer-events: none;
  ...
  opacity: 0.5;
  background:
    radial-gradient(40% 50% at 20% 25%, rgba(120, 130, 150, 0.14), transparent 60%),
    radial-gradient(35% 45% at 80% 20%, rgba(150, 160, 175, 0.12), transparent 60%),
    radial-gradient(45% 55% at 65% 80%, rgba(170, 180, 195, 0.10), transparent 60%),
    radial-gradient(40% 50% at 30% 75%, rgba(100, 110, 130, 0.10), transparent 60%);
  background-size: 200% 200%, 200% 200%, 200% 200%, 200% 200%;
  animation: smooth-light-aurora-drift 24s ease-in-out infinite;
}

.smooth-light-theme-background__particles {
  position: absolute;
  inset: 0;
  z-index: 3;
  pointer-events: none;
...
.smooth-light-theme-background__radial-soft-center {
  position: absolute;
  top: 0;
  left: 50%;
  width: 100%;
  ...
  z-index: 4;
  pointer-events: none;
  background: radial-gradient(ellipse at top, rgba(0, 0, 0, 0.03) 0%, transparent 60%);
}

.smooth-light-theme-background__hero-readability-overlay {
  position: absolute;
  inset: 0;
  pointer-events: none;
  background: radial-gradient(ellipse 90% 70% at 50% 45%, rgba(255, 255, 255, 0) 0%, rgba(244, 245, 247, 0.35) 100%);
}

.smooth-light-theme-background__section-surface {
  background-color: #f8f9fa;
}

.smooth-light-theme-background__dot-grid {
  background-image: radial-gradient(rgba(0, 0, 0, 0.08) 1.5px, transparent 1.5px);
  background-size: 24px 24px;
}

.smooth-light-theme-background__line-grid {
...
@keyframes smooth-light-aurora-drift {
  0%, 100% {
...
function SmoothLightBackgroundStyles() {
  return <style>{LIGHT_BACKGROUND_CSS}</style>;
}

export function LightSmokeBackground({
  speed = 0.08,
  intensity = 0.5,
  grainStrength = 0.03,
  vignette = 1.8,
  ...
  useEffect(() => {
    ...
    gl.shaderSource(vs, LIGHT_VERT);
    gl.compileShader(vs);
    if (!gl.getShaderParameter(vs, gl.COMPILE_STATUS)) return;

    gl.shaderSource(fs, LIGHT_FRAG);
    gl.compileShader(fs);
...
export function LightParticleBackground({ className = LIGHT_PARTICLE_CANVAS_CLASS }: LightParticleBackgroundProps) {
      ...
      for (let i = 0; i < particles.length; i += 1) {
        ...
        if (mouse.x > 0 && mouse.y > 0) {
          ...
          if (dist < mouse.radius) {
            ...
            ctx.strokeStyle = `rgba(${rgb}, ${0.2 * force})`;
            ctx.lineWidth = 0.7;
            ctx.stroke();
          }
        }
  ...
  return <canvas id="particles-canvas-light" ref={canvasRef} className={className} aria-hidden="true" />;
}

export function LightAuroraField() {
  return <div className="smooth-light-theme-background__aurora" aria-hidden="true" />;
}

export function LightHeroReadabilityOverlay() {
  return <div className="smooth-light-theme-background__hero-readability-overlay" aria-hidden="true" />;
}

export default function SmoothLightThemeBackground({
  className = '',
  includeAurora = true,
  includeParticles = false,
  includeSmoke = true,
  includeRadialGlow = true,
}: SmoothLightThemeBackgroundProps) {
  return (
    <div className={`smooth-light-theme-background fixed inset-0 -z-20 pointer-events-none overflow-hidden ${className}`.trim()} aria-hidden="true">
      <SmoothLightBackgroundStyles />
      <div className="smooth-light-theme-background__base absolute inset-0" />
      {includeAurora && <LightAuroraField />}
      {includeSmoke && <LightSmokeBackground />}
      {includeParticles && <LightParticleBackground className="smooth-light-theme-background__particles" />}
      {includeRadialGlow && <div className="smooth-light-theme-background__radial-soft-center" />}
    </div>
  );
}
